...
//    DateTime time = DateTime.parse(date);
  ...
  static homeWorkToString(String type) {
    if (type == 'FIRE_WORK') return '动火作业';
    if (type == 'ELECTRIC_WORK') return '临时用电';
    if (type == 'LIMIT_WORK') return '有限空间作业';
    if (type == 'CLIMB_WORK') return '登高作业';
    if (type == 'FRAG_WORK') return '零星工程';
    return type;
  }

  static typeToString(String type, {String procTmplName}) {
    // C 通用
    ...
    // FC 线下培训 | 安全会务
    if (type == 'FC_AUDIT') return procTmplName == 'SAFE_MEETING' ? '会务审核' : '培训审核';
    if (type == 'FC_START') return procTmplName == 'SAFE_MEETING' ? '会务开始' : '培训开始';
    if (type == 'FC_END') return procTmplName == 'SAFE_MEETING' ? '会务结束' : '培训结束';
    if (type == 'FC_SIGN') return procTmplName == 'SAFE_MEETING' ? '会务签到' : '培训签到';
    if (type == 'FC_UPLOAD') return procTmplName == 'SAFE_MEETING' ? '会务上传记录' : '培训上传记录';
    // OC 线上培训
